...
class _PageViewAvatarState extends State<PageViewAvatar> {
  ...
  void saveImage(BuildContext context) async {
    try {
      if (widget.imagePicker != null) {
        final bytes = await widget.imagePicker!.readAsBytes();
        final fileName = widget.imagePicker!.path
        ...
        await _downloadAndSave(widget.imageNetwork!, context);
      } else if (widget.listImageNetwork != null &&
          widget.listImageNetwork!.isNotEmpty) {
        await _downloadAndSave(widget.listImageNetwork!.last, context);
      } else {
        debugPrint('Failed to save image. No image source provided.');
